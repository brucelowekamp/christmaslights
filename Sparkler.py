...
class Sparkler(object):
  @staticmethod
  def SetArgs(parser):
    parser.add_argument('--sparkfrac', type=float, default=0.002, help="proportion of pixels sparkling")
  ...
  def Twinkle(display, colorfunc=None, steps=None, frac=None):
    return Sparkler(display, (colorfunc or WhiteColor), steps=(steps or Options.sparksteps), fraction=(frac or Options.sparkfrac), reverse=True, fullBright=True)
  ...
  def _add_flasher(self):
    ...
    self._flashers.append(Sparkler.Flasher(self._display, pixel,
                                           self._Colorfunc, self._Steps, self._reverse, self._fullBright))
